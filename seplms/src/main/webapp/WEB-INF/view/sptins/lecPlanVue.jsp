...
<script type="text/javascript">
	...
	function checkNotEmptyName(arr) {
		...
		for(var i = 0; i < weekVal.length; i++){
			if(weekVal[i].week_goal.trim() == "" || weekVal[i].week_goal == ""){
				alertMsg = arr[1][1];
				swal(alertMsg);
				return false;
			}
			
			if(weekVal[i].week_ctt.trim() == "" || weekVal[i].week_ctt == ""){
				alertMsg = arr[2][1];
				swal(alertMsg);
				return false;
			}
				...
				<div style="width:100%; height:200px; overflow:auto; margin-top: 5px; border: solid 2px #e5e5e5;">
					<table class="col">
						...
						<tbody id="weeklyListArea">
							<template v-for="list in weekList">
								<tr>
									<td>{{ list.weekly_no }}</td>
									<td>
										<textarea name="week_goal" v-model="list.week_goal" rows="5" cols="30" :style="vStyle" :readonly="readonly"></textarea>
									</td>
									<td>
										<textarea name="week_ctt"  v-model="list.week_ctt" rows="5" cols="30" :style="vStyle" :readonly="readonly"></textarea>
									</td>
								</tr>
							</template>
						</tbody>
