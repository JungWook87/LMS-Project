...
<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			userCombo("usr","userlist","all","A","");   // usertype별 사용자 정보    "A" <== Usertype     value : loginID 사용자ID
			
			selectComCombo("lecbyuser","lecbyuserlist","all","","");   // 현재 로그인한 사용자가 강의하는 강의    value : lec_cd lec_nm
			
			// 1) 강사명 (tb_lec > loginID 제외 조건 // tb_userinfo > loginID,, name)   
			userCombo("lusr","luserlist","all","N","");   // 강의 유무별  사용자(강사) 정보    "Y"(강의중")   "N"(강의없음)  ""(전체)   <== 구분자     value : loginID 사용자ID
			
			// 2) 강의실 (tb_cls_room)
			selectComCombo("cls","clslist","all","","");   // 강의실 목록    value : cls_room_cd 
			
			// 3) 현재 종료되지 않은 강의명(tb_lec > lec_cd, lec_nm ,,lec_yn 조건)  
			selectComCombo("lecyn","lecynlist","all","N","");   // 강의실 목록    value : lec_cd 
			
			// 4) 장비목록 (tb_equ_list   equ_cd, equ_nm)
			selectComCombo("equ","equlist","all","","");   // 장비 목록    value : equ_cd 			
			
			// 5) 실업 분류 (재직자, 실업자 // tb_job_ctg)
			selectComCombo("job","joblist","all","","");   // 실업 분류 목록    value : ctg_cd 		
			
			// 6) 문제타입 (자바초보, 자바중급, 자바고급... // tb_que_type 대상 테이블)
			selectComCombo("ttype","ttypelist","all","","");   // 문제타입 목록    value : test_que_type 	
			
			// 7) 테이블명 : tb_testinfo 시험코드 test_cd int (pk) 시험지명 test_paper_nm (varchar100)
			selectComCombo("test","testlist","all","","");   // 테스트 목록    value : test_cd 	
	...
	function fListInf(currentPage) {
		...
		var param = {
				...
				,   title : noticeareavar.stitle
				,   from_date : noticeareavar.sdate
				,   to_date : noticeareavar.edate
				,   selval : noticeareavar.selval
		}
		
		var resultCallback = function(data) {
			console.log("fListInf  !!!!!!!!!!!!!!!!! : " + JSON.stringify(data));
	...
	function fListInfResult(data, currentPage) {
		...
		noticeareavar.boardtype = data.boardtype;
	    ...
	    noticeareavar.listInfPagination = paginationHtml;
				...
				<li class="contents">
					...
					<div class="content" style="margin-bottom:20px;">
						    ...
						    <table>
						        ...
						        <tbody>
                                     ...
                                     <tr> 
						                  ...
						                  <td> 
						                      <select id="equlist" name="equlist" ></select>
						                  </td>		
						                  <td> 
						                       실업구분
						                  </td>						                      					                  
						                  <td> 
						                     <select id="joblist" name="joblist" ></select>
						                  </td>
						                  <td> 
						                      문제타입
						                  </td>
						                  <td> 
						                     <select id="ttypelist" name="ttypelist" ></select>
						                  </td>
						             </tr>						        
						             <tr> 
						                  <td> 
                        ...
                        <div id="vuevar">
	                       ...
	                       <p class="conTitle" style="margin-bottom: 1%;">
								<span>9. 공지 사항 샘플   </span> 
								<span class="fr"> 
								       종류 <select id="selval" name="selval" v-model="selval">		
								                  <option value="">전체</option>						   
								                  <template v-for="key in boardtype">
								                       <option :value="key.dtl_cod">{{ key.dtl_cod_nm }}</option>
								                  </template>
								               </select>
								
									    제목 <input type="text"  id="stitle" name="stitle"  v-model="stitle" />
									    기간 <input type="date"  id="sdate" name="sdate"  v-model="sdate" />
									           <input type="date"  id="edate" name="edate"  v-model="edate" />
									    <a class="btnType blue" href="javascript:fListInf()" name="modal"><span>조회</span></a>
								</span>
							</p>					        
								...
								<div class="bootstrap-table">
												...
												<table class="col">
													...
													<tbody id="listInf" v-for="(item,index) in listitem" v-if="listitem.length">
													      <tr @click="detailview(item.noticeNo)">
															...
															<td>{{ item.brd_veiws_cnt }}</td>	
															<td>{{ item.brd_delete_yn }}</td>	
															<td>{{ item.ctg_cd }}</td>	
															<td>{{ item.ans_yn }}</td>	
													      </tr>
													</tbody>
								   ...
								   <div class="paging_area"  id="listInfPagination" v-html="listInfPagination"> </div>
								</div>			   
