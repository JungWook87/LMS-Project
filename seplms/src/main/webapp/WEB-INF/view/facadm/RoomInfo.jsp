...
/* 		 콤보박스 있을 경우 사용 
	...
	function fnSearchChange(){
	    var selectedValue = $("#searchSel").val(); // 선택된 value 값을 가져옵니다.
	    console.log($("#searchSel").val())
	    if (selectedValue === "useYN") {
	        var inhtml = "<select id='searchText'></select>";
	        $("#searchSelBox").empty().append(inhtml);
	        comcombo("USEYN", "searchText", "sel", "");
	    } else {
	        var inhtml = "<input type='text' id='searchText' name='searchText' style='width: 150px; height: 25px;' />";
	        $("#searchSelBox").empty().append(inhtml);
	    }
	}
    ...
    function fnPopUpInt(data) {
    	...
    	if(data == null || data == undefined || data == "") {
        	...
        	$("#cls_room_use_yn").val(data.cls_room_use_yn).attr("selected", "selected"); // 강의실 사용유무
        	$("#temUsingYN").val(data.cls_room_use_yn) // 강의실 사용유무
        	$("#cls_room_seat").val(data.cls_room_seat); // 강의실 좌석수
        	$("#cnt_equ").val(data.cnt_equ); // 강의실 장비 개수
...
///////////////////////////////하단 모달 작업 앞에 dtl또는 Dtl넣어서 구분 ///////////////////////////////////////////////////////////////
	...
	function fnListDtlResult(data, currentPage) {
		...
		var paginationHtml = getPaginationHtml(currentPage, totalCntRoomDtl, pageSizeDtl, pageBlockDtl, 'fnListRoomDtl', [cls_room_cd, cls_room_nm]);
	...
	function fnSaveDtl() {
    	...
    	var saveCallBack = function(returnValue) {
    		...
    		if(returnValue.resultMsg == "저장" || returnValue.resultMsg == "수정" || returnValue.resultMsg == "삭제") {
    			...
    			if($("#action").val() == "I") {
    				fnSearchList();
    				fnListRoomDtl($("#currentPageDtl").val(), $("#selroomcd").val(), $("#tmpRoomNm").val());
    			} else if($("#action").val() == "U") {
    				fnSearchList($("#currentPage").val());
    				fnListRoomDtl($("#currentPageDtl").val(), $("#selroomcd").val(), $("#tmpRoomNm").val());
    			} else if($("#action").val() == "D") {
...
<form id="myForm" action="" method="">
	...
	<input type="hidden" id="tmpRoomNm" value=""> <!-- 강의실명 -->
				...
				<table class="row">
					...
					<tbody>
						<tr>
							...
							<td> 
								<input type="hidden" id="temUsingYN"/>
								<select id="cls_room_use_yn" name="cls_room_use_yn" onChange='javascript:fnUsingCheck()'>
